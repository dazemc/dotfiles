...
1. [S] Resolve the dead screenshot-dir vars. `conf/envars.lua` sets
...
2. [S] Fix the tmux `w` window name. `tmux/.tmux.conf` binds `w` to
...
3. [S] Record the SSH access recipe and recovery path. `sshd_config` runs
